...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/zookeeper/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/zookeeper/*")
            ...
            #    "--include=src/saltext/zookeeper/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/zookeeper/config/schemas",
    )
